...
import os, re, jinja2, feedparser, arxiv, google.generativeai as genai, time, random, tldextract
import requests, pickle, asyncio, hashlib, json
...
CURATED_DATA_FILE    = 'curated_items.json'     # export for admin
...
RSS_FEEDS_BASE = [
    ...
    # Corporate newsrooms
    "https://blogs.nvidia.com/feed/","https://about.fb.com/news/category/technology/ai/feed/",
    "https://about.google/press/rss/","https://aws.amazon.com/blogs/aws/category/artificial-intelligence/feed/",
    "https://blogs.microsoft.com/feed/","https://news.microsoft.com/source/topics/ai/feed/",
    "https://www.ibm.com/blogs/research/feed/","https://www.salesforce.com/news/feed/",
    "https://blogs.oracle.com/rss","https://www.apple.com/newsroom/rss-feed.rss","https://www.adobe.com/blog/feed",

    # Deals
    "https://news.crunchbase.com/sections/ai/feed/",

    # AI pubs
    "https://dailyai.com/feed/","https://www.marktechpost.com/feed/","https://emerj.com/feed/",
...
def fetch_feed(feed_url):
    try:
        resp = requests.get(feed_url, timeout=10); resp.raise_for_status()
        feed = feedparser.parse(resp.content)
    except Exception as e:
        print(f"Error fetching {feed_url}: {e}"); feed = {'entries': []}
...
async def async_get_headline_and_score(title, summary):
    ...
    prompt = f"""
    Analyze title+summary. 1) Sensational score 1-20. 2) Rewrite to viral, factual <15 words (company first). 3) Tech importance 0-10.
    ...
    try:
        ...
        if len(parts) >= 3:
            return parts[1].strip().replace('*',''), int(parts[0].strip()), int(parts[2].strip())
        return fallback, 5, 0
    except Exception as e:
        print(f"Gemini error: {e}"); await asyncio.sleep(2); return fallback, 5, 0
...
if __name__ == "__main__":
    ...
    # Group by normalized title; pick best; optionally mirror
    groups = {}
    for a in all_articles:
        title = a.get('title',''); norm = normalize_title(title) or (domain_of(a.get('url','')) + normalize_title(title))
    ...
    if not DISABLE_OVERRIDES and overrides_map:
